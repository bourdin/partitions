...
int main (int argc, char ** argv) {
    ...
    SavePNG = PETSC_TRUE;
    ierr = PetscOptionsGetTruth(PETSC_NULL, "-savePNG", &SavePNG, PETSC_NULL); CHKERRQ(ierr);
    SaveTXT = PETSC_TRUE;
    ...
    for (level=0; level<user.numlevels; level++){
        ...
        for (it=0; it<maxit; it++){
            ...
            if (it%modsave == 0){
                ...
                if (SavePNG == PETSC_TRUE){
                    ierr = PetscLogStagePop(); CHKERRQ(ierr);
                    sprintf(filename, "%s%.3d-level%d%s", u_prfx, myrank, level, png_sfx);
